from .scraper import fetch_and_parse_sp500, fetch_and_parse_nasdaq100, fetch_and_parse_sp100
...
SUPPORTED_FIELDS_SP100 = {'symbol', 'name', 'sector'}
_CACHE = {}
...
def get_sp100(return_type='list', fields=None):
    if return_type not in ('list', 'dict'):
        raise ValueError("Invalid return_type. Must be 'list' or 'dict'.")
    if fields is None:
        fields = ['symbol']
    for field in fields:
        if field not in SUPPORTED_FIELDS_SP100:
            raise ValueError(f"Unsupported field: {field}")
            
    if 'sp100' not in _CACHE:
        _CACHE['sp100'] = fetch_and_parse_sp100()
